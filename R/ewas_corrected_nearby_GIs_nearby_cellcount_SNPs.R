## Perform EWAS per genetic instrument (GI), while correcting for neighboring GIs + nearby SNPs known to influence white blood cell composition
# Note that this script runs per chromosome (chr.. should be supplied as argument to this script)
# The submit_ewas_corrected_nearby_GIs_nearby_cellcount_SNPs.R script was used to run this script for each chromosome
...
# Load genotypes
load(sprintf("%sdata/genotypes_cellcounts.RData", direc))
genotypes <- genotypes[,cvrt$imputation_id]

snps.locs <- GRanges( seqnames = paste0("chr", SNPLocations_cellcounts$CHROM),
    ranges = IRanges(
        start = SNPLocations_cellcounts$POS,
        end = SNPLocations_cellcounts$POS
    ),
    RS_ID = SNPLocations_cellcounts$RS_ID,
    POS_ID = SNPLocations_cellcounts$POS_ID)

# Correlation among the cellcount SNPs and the genetic instruments
cors <- cor(grs, t(genotypes))

# Some genetic instruments are strongly correlated with SNPs that are associated with cell composition
# Remove these, because of collinearity
genes_cellcounts <- unique(rownames(which(abs(cors) > 0.95, arr.ind = TRUE)))
save(genes_cellcounts, file = sprintf("%soutput.genome.wide/ewas.corrected.nearby.gi.snps.cellcounts/genes_cellcounts.RData", direc)) # Save
grs <- grs[,!(colnames(grs) %in% genes_cellcounts)] # Remove these genes

#
all.genes <- colnames(grs)
genes <- genes[!(genes %in% genes_cellcounts)]
probes.locs <- probes.locs[probes.locs$gene %in% all.genes]
...
run <- function(current.gene) {
        ...
        # Get nearby SNPs that influence cellcounts
        corr.snps <- subsetByOverlaps(snps.locs, probes.locs[probes.locs$gene == current.gene], maxgap = 1e6)
        corr.snps <- t(genotypes[corr.snps$POS_ID,, drop=FALSE])

        # Perform LD pruning (some SNPs are in strong LD)
        # Use the findCorrelation function from the caret package to perform pruning
        if(ncol(corr.snps) > 1) {
            cors <- cor(corr.snps)
            test <- findCorrelation(cors, cutoff = 0.95)
            if(length(test) > 0) {
            corr.snps <- corr.snps[,-test, drop = FALSE]
                } else {
                    corr.snps <- corr.snps
                }
        }
        ...
        # Make model matrices (if )
        if(ncol(corr.genes) > 0 & ncol(corr.snps) > 0) {

            # model matrix
            Z <- model.matrix(~ corr.genes + corr.snps +
                Sampling_Age +
                Sex +
                Neutrophils +
                Lymphocytes +
        ...
        } else if(ncol(corr.genes) > 0 & ncol(corr.snps) == 0) {
            Z <- model.matrix(~ Sampling_Age + corr.genes +
                Sex +
                Neutrophils +
                Lymphocytes +
                Monocytes +
              ...
              Z <- model.matrix(~ Sampling_Age + corr.snps +
                Sex +
                Neutrophils +
                Lymphocytes +
                Monocytes +
        ...
        write_tsv(me,path = sprintf("%soutput.genome.wide/ewas.corrected.nearby.gi.snps.cellcounts/output/%s.%s.txt", direc, chr, current.gene), col_names=TRUE)
        nr.covariates <- ncol(Z)
        write.table(nr.covariates, file = sprintf("%soutput.genome.wide/ewas.corrected.nearby.gi.snps.cellcounts/nr_covariates/%s.txt", direc,
